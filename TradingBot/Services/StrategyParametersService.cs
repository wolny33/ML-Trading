...
namespace TradingBot.Services
{
    public interface IStrategyParametersService
    {
        ...
        public Task<StrategyParametersConfiguration> SetParametersAsync(int maxStocksBuyCount, int minDaysDecreasing, 
            decimal topGrowingSymbolsBuyRatio, CancellationToken token = default);
    }
    public sealed class StrategyParametersService : IStrategyParametersService
    {
        ...
        public async Task<StrategyParametersConfiguration> SetParametersAsync(int maxStocksBuyCount, int minDaysDecreasing, decimal topGrowingSymbolsBuyRatio, CancellationToken token = default)
        {
            await using var context = await _dbContextFactory.CreateDbContextAsync(token);
            var entity = await context.StrategyParameters.FirstOrDefaultAsync(token);
            if (entity is null)
