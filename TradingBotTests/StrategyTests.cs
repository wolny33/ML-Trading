...
public sealed class StrategyTests
{
    ...
    public async Task ShouldCorrectlyGenerateTradeActions()
    {
        ...
        var PLUGPrice = Math.Round((PLUGCurrentPrice + _predictions[new TradingSymbol("PLUG")].Prices[0].LowPrice) / 2,
            2);
        var PLUGQuantity = (int)Math.Floor(_assets.Cash.BuyingPower *
            (decimal)_strategyParameters.Basic.TopGrowingSymbolsBuyRatio / PLUGPrice);
        tradeActions.Should().ContainEquivalentOf(new TradingAction
        {
            Id = Guid.NewGuid(),
            CreatedAt = time,
        ...
        var SOXSPrice = Math.Round((SOXSCurrentPrice + _predictions[new TradingSymbol("SOXS")].Prices[0].LowPrice) / 2,
            2);
